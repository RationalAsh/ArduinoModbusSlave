#ifndef MODBUSRTULIB_H
#define MODBUSRTULIB_H
...
int encodeRTUFrame(const uint8_t *data, WORD dlen, uint8_t *frame, uint8_t slave_addr, uint8_t function_code)
{
    ...
    frame[i+2] = crc_val & 0x00FF;
    frame[i+2+1] = (crc_val >> 8) & 0x00FF;
